...
### 1D Test Case : One-phase Stefan Problem (RK2 Time Integration)
...
# Solve the problem using the RK2 METHOD with 2nd-order time integration
println("Solving the Stefan problem with RK2 time integration (2nd order)...")
solver, xf_log = solve_MovingLiquidDiffusionUnsteadyMono_RK2!(
...
function plot_temperature_comparison(
    ...
    t_final::Float64,
    xf_final::Float64;
    save_path::String="temperature_comparison_rk2.png"
)
    ...
    ax = Axis(
        ...
        xlabel = "Position (x)",
        ylabel = "Temperature",
        title = "Stefan Problem (RK2 Method): Temperature Comparison at t = $t_final"
    )
...
function plot_interface_position(
    ...
    lambda=1.0,
    vertical_offset=0.0,
    save_path::String="interface_position_rk2.png"
)
    ...
    ax = Axis(
        ...
        title = "Stefan Problem (RK2 Method): Interface Position vs Time"
    )
...
# Summary errors
println("\n=== Summary of Errors at t=$(Tend) (RK2 METHOD) ===")
