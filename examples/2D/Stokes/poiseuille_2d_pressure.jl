...
"""
2D Stokes Poiseuille flow (steady, fully developed).

We impose periodicity in x (no inlet/outlet traction) and drive the flow with a
constant body force `G` so the channel reaches the analytical parabolic profile
`u_x(y) = (G/(2μ)) y (Ly - y)` without boundary-layer artifacts.
"""
...
μ = 1.0
Umax = 1.0
G = 2 * μ * Umax / (Ly^2)  # body force amplitude (acts in +x)
parabola = (x, y) -> 0.5 * G / μ * (y - y0) * (Ly - (y - y0))
...
ax1 = Axis(fig[1,1], xlabel="u_x", ylabel="y", title="Pressure-driven Poiseuille: mid x profile")
lines!(ax1, ux_profile, ys, label="numerical")
lines!(ax1, ux_analytical, ys, color=:red, linestyle=:dash, label="analytical")
axislegend(ax1, position=:rb)
display(fig)
save("stokes2d_poiseuille_pressure_profile.png", fig)
...
display(fig2)
save("stokes2d_poiseuille_pressure_fields.png", fig2)
...
Uy_max = maximum(abs, reshape(uωy, :))
println("Sanity: max |u_y| = ", Uy_max)
