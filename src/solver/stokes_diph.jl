...
function stokes1D_phase_unsteady_blocks(fluid::Fluid{1})
    data = stokes1D_phase_blocks(fluid)
    ρ = fluid.ρ
    mass_x = build_I_D(data.op_ux, ρ, data.cap_px) * data.op_ux.V
    return (; data..., mass_x)
end

function stokes3D_phase_unsteady_blocks(fluid::Fluid{3})
    data = stokes3D_phase_blocks(fluid)
    ρ = fluid.ρ
    mass_x = build_I_D(data.op_ux, ρ, data.cap_px) * data.op_ux.V
    mass_y = build_I_D(data.op_uy, ρ, data.cap_py) * data.op_uy.V
    mass_z = build_I_D(data.op_uz, ρ, data.cap_pz) * data.op_uz.V
    return (; data..., mass_x, mass_y, mass_z)
end
...
function assemble_unsteady_stokes1D_diph!(s::StokesDiph,
                                          data_a,
                                          data_b,
                                          Δt::Float64,
                                          x_prev::AbstractVector{<:Real},
    ...
    u1ωx_prev = view(x_prev, off_a_uωx+1:off_a_uωx+nu)
    u1γx_prev = view(x_prev, off_a_uγx+1:off_a_uγx+nu)
    ...
    apply_velocity_dirichlet!(A, b, s.bc_u_a[1], s.fluid_a.mesh_u[1];
                              nu=nu, uω_offset=off_a_uωx, uγ_offset=off_a_uγx, t=t_next)
    apply_velocity_dirichlet!(A, b, s.bc_u_b[1], s.fluid_b.mesh_u[1];
                              nu=nu, uω_offset=off_b_uωx, uγ_offset=off_b_uγx, t=t_next)
...
function assemble_unsteady_stokes3D_diph!(s::StokesDiph,
                                          data_a,
                                          data_b,
                                          Δt::Float64,
                                          x_prev::AbstractVector{<:Real},
    ...
    mass_ax_dt = (1.0 / Δt) * data_a.mass_x
    mass_ay_dt = (1.0 / Δt) * data_a.mass_y
    mass_az_dt = (1.0 / Δt) * data_a.mass_z
    mass_bx_dt = (1.0 / Δt) * data_b.mass_x
    mass_by_dt = (1.0 / Δt) * data_b.mass_y
    mass_bz_dt = (1.0 / Δt) * data_b.mass_z
    θc = 1.0 - θ
    ...
    u1ωy_prev = view(x_prev, off_a_uωy+1:off_a_uωy+nu_ay)
    u1γy_prev = view(x_prev, off_a_uγy+1:off_a_uγy+nu_ay)
    u1ωz_prev = view(x_prev, off_a_uωz+1:off_a_uωz+nu_az)
    u1γz_prev = view(x_prev, off_a_uγz+1:off_a_uγz+nu_az)
    ...
    rhs_mom_az = mass_az_dt * u1ωz_prev
    rhs_mom_az .-= θc * (data_a.visc_z_ω * u1ωz_prev + data_a.visc_z_γ * u1γz_prev)
    rhs_mom_az .+= load_az
    ...
    rhs_mom_bz = mass_bz_dt * u2ωz_prev
    rhs_mom_bz .-= θc * (data_b.visc_z_ω * u2ωz_prev + data_b.visc_z_γ * u2γz_prev)
    rhs_mom_bz .+= load_bz
...
function solve_StokesDiph_unsteady!(s::StokesDiph; Δt::Float64, T_end::Float64,
    ...
    if N == 1
        data_a = stokes1D_phase_unsteady_blocks(s.fluid_a)
        data_b = stokes1D_phase_unsteady_blocks(s.fluid_b)
        nu = data_a.nu
        np_a = data_a.np
        np_b = data_b.np
        Ntot = 4 * nu + np_a + np_b
        assembler = assemble_unsteady_stokes1D_diph!
    elseif N == 2
        ...
        assembler = assemble_unsteady_stokes2D_diph!
    elseif N == 3
        data_a = stokes3D_phase_unsteady_blocks(s.fluid_a)
        data_b = stokes3D_phase_unsteady_blocks(s.fluid_b)
        nu_ax = data_a.nu_x; nu_ay = data_a.nu_y; nu_az = data_a.nu_z
        np_a = data_a.np;     np_b = data_b.np
        Ntot = 4 * (nu_ax + nu_ay + nu_az) + np_a + np_b
        assembler = assemble_unsteady_stokes3D_diph!
    else
        error("StokesDiph unsteady solver implemented for 1D/2D/3D (N=$(N)).")
    end
    ...
    while t < T_end - 1e-12 * max(1.0, T_end)
        ...
        assembler(s, data_a, data_b, dt_step, x_prev, t, t_next, θ)
        solve_stokes_linear_system!(s; method=method, algorithm=algorithm, kwargs...)
